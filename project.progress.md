...
**✅ SEAL A COMPLETE** (part 2 = commit bdlmt4xel): pane Evidence filled via the DRIVEN pane-independence contract (behavioral-contracts.sh `pane-independence`: reach editor true last line → open change diff via Ctrl+G/`o` → close → editor reaches SAME last line + same extent; drive-verified editorScrollTop 85 before/after, PLINE-119 both). Added `showingDiff` status field. `check_invariants.mjs --all` AND `--refs` exit 0 and are now HARD-BLOCKING steps in merge-gate.sh — the contract layer is ENFORCED, not just measured. Closes census ranked-hole #1.

**NEXT SEALS (from SEAL-CHECKLIST-verified.md):** SEAL B — 13 stray `export function`s → Static (11 in editor.coordinates.ts→new EditorCoordinates; boot@Bootstrap.ts:52; buildRootView@RootView.ts:91) + harden conventions-gate #1 to an AST check (drop the whole-file allowlist; expandReplacement@FindInBuffer is NOT a violation — private). SEAL C — context-menu modal claims (click-through-blocked / one-menu / keyboard-captured) are undriven; selection RENDERING is state-asserted not frame-asserted (add a bg-color `paintedcols()` frame check).
**Then the queued FEATURE work:** clickpath/click-completeness gate + activity-bar clickable Search (HIGH, user "make sure of it"); dirty-dot content-hash; Alt+Delete deletePreviousWord=[wordLeft(P),P]; FuzzyPicker distillation; creative track (git gutter diff showcase reusing DiffAlignment).
