...
LEGACY_STATIC_ALLOWLIST="CommandRegistry.ts|editor.coordinates.ts|scroll-momentum.ts|commands.defaults.ts|keybindings.defaults.ts|keybindings.mac.ts|theme.palettes.ts|RootView.ts|Static.ts"
