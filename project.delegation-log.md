...
- feat-search-integ · tip 7c7d106 · merged-into main (7c7d106) · 2026-07-23 · integrated feat-search-mouse @ 4204798 (mouse-first QuickOpen + FindBar: highlight/no-arrow/clickable buttons/case toggle/live path-navigator) by CHERRY-PICK onto current main (single commit, avoids the stale-a1ceb09-base reversions). Keep-both conflict resolution across the hot files: RootView (buildRootView takes panelHost/toggleTerminal AND activateQuickOpenSelection/revealFindMatch), Bootstrap (all four wired; DEDUPED revealFindMatch which HEAD+search both defined), Theme+ThemeIcons (activity/settings/terminal icon ladders alongside new find/alert ladders); keybindings applied clean. 19 search unit tests + smoke-search-mouse ALL-PASS. Then merged origin (4fcbf9f = coordinator's part-4 de-fieldset + smoke-markdown probe fix) — clean auto-merge (disjoint RootView regions); re-confirmed smoke-search-mouse + smoke-markdown ALL-PASS in the combined tree. finished/ tag pushed. NOTE: the earlier markdown "coupling" was a TEST-PROBE issue (probe located the pane by title text), fixed by the coordinator's part-4 — app always keyed find-source off the document path.
- feat-buffertab-restyle · tip 3e12f46 · merged-into main (3e12f46) · 2026-07-23 · buffer-tab restyle, 3 of 4 parts: (1) breadcrumb filename (project › dir › file, capped, leading dirs collapse to …, filename always kept) via a dedicated Breadcrumb.ts capability (class $Breadcrumb + namespace Breadcrumb{Class=Static}, file-name-follows-class, 8 unit tests); (2) tier-aware powerline separator between tabs (nerd  → unicode ❯ → ascii >); (3) first-tab 1-cell gap off the splitter + idle accent tint. smoke-tabs gains breadcrumb+separator assertions (tmux capture — FrameProbe remaps ›). PART 4 (remove editor-pane filename border-legend) DEFERRED + reverted: blanking editorArea.title deterministically breaks smoke-markdown (find source=''/paste no-op) — the find pane's SOURCE IDENTITY keys off the display title; proper fix = decouple identity to the document path (coordinator owns, bundled w/ status-bar button). Note in project.handoff.md + memory reference-editorarea-title-markdown-coupling. finished/ tag pushed.
- fix-activitybar-accent · tip 3d3ed32 · merged-into main (3d3ed32) · 2026-07-23 · badge-refinement follow-up: part-1 dropped the active-item accent to the bottom row, misaligning it from the icon; now the badge sits ALONE on the top row (placeholder above the icon) and the accent bar is back on the SAME row as the ICON (aligned, reads as the icon's selection highlight). smoke-activitybar asserts accent-on-icon-row + badge-on-row-above. finished/ tag pushed.
- feat-undo-unchanged · tip dd52e04 · merged-into main (dd52e04) · 2026-07-23 · #5 undo-to-original reads UNCHANGED: dirty was a sticky boolean set true on every edit; now TextDocument captures a content signature (lineCount:FNV-1a) at each clean point (load/markSaved), and Editor.performUndo/Redo set dirty=!matchesSaved() so an undo/redo landing on the saved content is clean again (edits stay eager-dirty fast-path). VERIFIED the user's actual symptom — the RENDERED ● tab dot clears, not just the flag (active tab reads buffer.dirty LIVE per OpenBufferSet.tabs line 80, so the reactive fix clears the dot with no cache resync); smoke-editor asserts the dot CELL non-space→space (FrameProbe remaps ●). 3 unit tests. Also installed the terminal merge's declared-but-missing @xterm/headless@6 into shared node_modules (the terminal feature would have crashed without it). finished/ tag pushed.
