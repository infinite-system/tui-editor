...
test('inline code keeps a plain word but collapses a PATH to its last segment', () => {
  expect(speak('call `render()` again')).toBe('call render() again');
  expect(speak('edit `/tmp/wt-voice/src/main.ts`')).toBe('edit main.ts');
});
...
test('the reported babble case reads cleanly', () => {
  const input = 'I ran `/tmp/wt-voice/scripts/merge-gate.sh` and it passed. See `SpeakableText.ts`.';
  expect(speak(input)).toBe('I ran merge-gate.sh and it passed. See SpeakableText.ts.');
});
