...
describe('missingRanges', () => {
  test('all missing → one range covering the window', () => {
    expect(missingRanges(new Set(), 0, 5)).toEqual([{ offset: 0, length: 5 }]);
  });
  test('all loaded → no ranges', () => {
    expect(missingRanges(new Set([0, 1, 2, 3, 4]), 0, 5)).toEqual([]);
  });
  test('a gap in the middle produces one range for the gap only', () => {
    // loaded 0,1,4; window [0,5) → missing 2,3
    expect(missingRanges(new Set([0, 1, 4]), 0, 5)).toEqual([{ offset: 2, length: 2 }]);
  });
  test('two separate gaps → two ranges (batched, not per-row)', () => {
    // loaded 1,4; window [0,6) → missing 0 | 2,3 | 5
    expect(missingRanges(new Set([1, 4]), 0, 6)).toEqual([
      { offset: 0, length: 1 },
      { offset: 2, length: 2 },
      { offset: 5, length: 1 },
    ]);
  ...
  test('window offset away from zero', () => {
    ...
    expect(missingRanges(new Set([100, 101]), 100, 5)).toEqual([{ offset: 102, length: 3 }]);
  });
  test('negative start is clamped to 0', () => {
    expect(missingRanges(new Set(), -3, 4)).toEqual([{ offset: 0, length: 4 }]);
  });
});
